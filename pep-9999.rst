...
The ``Self`` type annotation is also useful for ``classmethod``s that return
...
See [#protocol-self-type]_ for details on the behavior of ``TypeVar``s bound
to protocols.
...
Note that we reject ``Self`` in ``staticmethod``s. ``Self`` does not add much
